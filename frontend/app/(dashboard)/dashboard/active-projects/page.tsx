...
export default function ActiveProjectsPage() {
  ...
  // Task Creation states inside workspace
  ...
  // Sync URL search parameters with workspace focus
  useEffect(() => {
    ...
    <div className="flex-grow flex flex-col space-y-6 font-sans text-slate-900 dark:text-white transition-colors duration-150">
      ...
      {selectedProject ? (
        ...
        <div className="space-y-6">
          {/* Pending Project Approval Queue (Visible to Assigned PM and HR) */}
          {pendingProjects.length > 0 && (
            <div className="bg-amber-50/40 dark:bg-amber-500/5 border border-amber-200/50 dark:border-amber-900/30 rounded-2xl p-6 text-left space-y-4">
              <div className="flex items-center gap-2 pb-2 border-b border-amber-250/20 dark:border-amber-900/20">
                <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse" />
                <h3 className="text-xs font-bold uppercase tracking-wider text-amber-800 dark:text-amber-400">
                  Pending Project Workspace Approvals ({pendingProjects.length})
                </h3>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {pendingProjects.map(proj => {
                  const isAssignedPm = sessionData.user.role === "PROJECT_MANAGER" && proj.manager?.id === sessionData.user.id;
                  const isHr = sessionData.user.role === "HR";
                  
                  return (
                    <div key={proj.id} className="bg-white dark:bg-zinc-950 p-5 rounded-xl border border-amber-200/20 dark:border-zinc-900 shadow-xs flex flex-col justify-between space-y-4">
                      <div className="space-y-1.5">
                        <div className="flex justify-between items-start">
                          <h4 className="text-sm font-bold text-slate-900 dark:text-white truncate">{proj.name}</h4>
                          <span className="bg-amber-100 text-amber-800 dark:bg-amber-500/10 dark:text-amber-400 text-[8px] font-bold uppercase px-1.5 py-0.5 rounded">
                            Pending PM
                          </span>
                        </div>
                        <p className="text-[11px] text-slate-500 dark:text-zinc-400 font-semibold line-clamp-2 leading-relaxed">
                          {proj.description || "No description provided."}
                        </p>
                      </div>

                      <div className="text-[10px] font-bold text-slate-400 space-y-1 pt-2 border-t border-slate-50 dark:border-zinc-900">
                        <div className="flex justify-between">
                          <span>Assigned PM:</span>
                          <span className="text-slate-700 dark:text-zinc-350">{proj.manager?.name || "Unassigned"}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Budget Parameters:</span>
                          <span className="text-slate-700 dark:text-zinc-350">
                            {new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(proj.budget)}
                          </span>
                        </div>
                      </div>

                      {/* Approve button only for assigned PM */}
                      {isAssignedPm && (
                        <button 
                          onClick={() => handleApproveProject(proj.id)}
                          className="w-full mt-2 bg-amber-500 hover:bg-amber-600 dark:bg-amber-600 dark:hover:bg-amber-700 text-white font-bold py-2 px-4 rounded-lg text-xs cursor-pointer shadow-xs transition-colors"
                        >
                ...
                {activeProjects.map(proj => (
                  ...
                  >
                    <div className="space-y-3">
                      ...
                      <p className="text-xs text-slate-450 dark:text-zinc-500 font-semibold line-clamp-3 leading-relaxed">
                        {proj.description || "No description provided."}
                      </p>
                    </div>

                    <div className="space-y-3">
                      <div className="grid grid-cols-3 gap-3 border-t border-slate-50 dark:border-zinc-900/60 pt-3 text-[10px] font-bold text-slate-400">
                        ...
                        <div className="col-span-2">
                          ...
                          <span className="text-slate-950 dark:text-white block">{formatTimeline(proj.startDate, proj.endDate)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
